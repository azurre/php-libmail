...
class Mail
{
    ...
    */
    ...
    @var array
    */
    var $priorities = ['1 (Highest)', '2 (High)', '3 (Normal)', '4 (Low)', '5 (Lowest)'];
    /*
    ...
    */
    ...
    var $names_email = []; // имена для email адресов, чтобы делать вид ("сергей" <asd@wer.re>)
    ...
    */
    function Subject($subject)
    {
        ...
        $this->xheaders['Subject'] = "=?" . $this->charset . "?Q?" . str_replace("+", "_",
                str_replace("%", "=", urlencode(strtr($subject, "\r\n", "  ")))) . "?=";
    ...
    function BuildMail()
    {
        ...
        foreach ($this->sendto as $key => $value) {
            ...
            if (strlen($this->names_email['To'][$value])) {
                $temp_mass[] = "=?" . $this->charset . "?Q?" . str_replace("+", "_", str_replace("%", "=",
                        urlencode(strtr($this->names_email['To'][$value], "\r\n", "  ")))) . "?= <" . $value . ">";
            } else {
                $temp_mass[] = $value;
            }
        }
        ...
        if ($this->smtp_on) {
            ...
            while (list($hdr, $value) = each($this->xheaders)) {
                if ($hdr == "From" and strlen($this->names_email['from'])) {
                    $this->headers .= $hdr . ": =?" . $this->charset . "?Q?" . str_replace("+", "_",
                            str_replace("%", "=", urlencode(strtr($this->names_email['from'], "\r\n",
                                "  ")))) . "?= <" . $value . ">\r\n";
                } elseif ($hdr == "Reply-To" and strlen($this->names_email['Reply-To'])) {
                    $this->headers .= $hdr . ": =?" . $this->charset . "?Q?" . str_replace("+", "_",
                            str_replace("%", "=", urlencode(strtr($this->names_email['Reply-To'], "\r\n",
                                "  ")))) . "?= <" . $value . ">\r\n";
                } elseif ($hdr != "BCC") {
    ...
    */
    function Send()
    {
        ...
        } else // если через smtp
        {
            ...
            fputs($smtp_conn,
                "MAIL FROM:<" . $this->xheaders['From'] . "> SIZE=" . strlen($this->headers . "\r\n" . $this->fullBody) . "\r\n");
